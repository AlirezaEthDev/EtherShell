...
export function createContractProxy(contract, provider, allAccounts) {
  return new Proxy(contract, {
    ...
    //   if (prop === 'interface') return target.interface;
    //   if (prop === 'runner') return target.runner;
    ...
    //       'value', 'nonce', 'gasLimit', 'gas', 'gasPrice',
    //       'maxFeePerGas', 'maxPriorityFeePerGas', 'chainId',
    //       'accessList', 'type', 'customData', 'from'
    //     ];
    ...
    //       console.warn("Warning: 'data' option is ignored. Calldata is encoded automatically by ethers.js.");
    //     }
    ...
    //       );
    //       if (!account) throw new Error(`Account ${options.from} not found in registered accounts`);
    //       if (!account.privateKey) throw new Error(`Account ${options.from} is a node-managed account`);
    ...
    //       else if (options.gas !== undefined) txOptions.gasLimit = options.gas;
    //       if (options.gasPrice !== undefined) txOptions.gasPrice = options.gasPrice;
    //       if (options.maxFeePerGas !== undefined) txOptions.maxFeePerGas = options.maxFeePerGas;
    //       if (options.maxPriorityFeePerGas !== undefined) txOptions.maxPriorityFeePerGas = options.maxPriorityFeePerGas;
    //       if (options.nonce !== undefined) txOptions.nonce = options.nonce;
    //       if (options.chainId !== undefined) txOptions.chainId = options.chainId;
    //       if (options.accessList !== undefined) txOptions.accessList = options.accessList;
    //       if (options.type !== undefined) txOptions.type = options.type;
    //       if (options.customData !== undefined) txOptions.customData = options.customData;
    //     }

    //     if (Object.keys(txOptions).length > 0) args.push(txOptions);

    //     const result = await method(...args);  // ethers functions are directly callable

    //     if (result && typeof result.wait === 'function') {
    //       const receipt = await result.wait();
    //       const tx = await provider.getTransactionReceipt(receipt.hash);
    //       const eventValues = eventOf(contract, tx);
    //       if (eventValues) receipt.eventValues = eventValues;
    //       return receipt;
    //     }
